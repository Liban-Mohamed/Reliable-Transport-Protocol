...
#returns: the created packet combining header + data, used for sending over the network
...
# Arguments: packet: the raw received packet (bytes)
# Returns: seq, ack, flags, window, data useful for the application logic
...
#what it does: Listens for client connection, receives file in chunks, handles connection establishment and teardown.
#Arguments:
#server's IP address to bind to. must be in dotted decimal format, like 10.0.1.2 for example
# port: server's port number (must be between 1024–65535)
# discard_seq: sequence number to simulate packet loss (int)
# returns: nothing. Server writes the received data to a file ('received_file') and prints throughput at the end.
def server(ip, port, discard_seq):
    ...
    while True:
        try:
            ...
            #if statement discarding a packet for testing retransmission
            if seq == discard_seq:
    ...
    #then finally it is divided by time to get mbits per second
...
#Description: Establishes connection with server, sends file data reliably using a sliding window, and handles teardown.
#Arguments:
#server's IP address to connect to and it must be dotted decimal, like 10.0.1.2
...
#returns:
# nothing. File is transmitted successfully and connection is closed properly.
def client(ip, port, filename, window_size):
    ...
    try:
        #the first sequence which is numbered zero is the syn packet which is the 1 in 0010. it also sends window size and the ip and port
    ...
    #two-way connection teardown with FIN
...
#description: parses command line arguments and starts either the server or client based on user input
# Arguments:
#-s/--server: flag to run as server
#-c/--client: flag to run as client
#-i/--ip: target IP address
#-p/--port: target port
#-f/--file: filename (client)
#-w/--window: sliding window size (client)
#-d/--discard: sequence number to discard (server)
#returns: nothing. Executes appropriate role (server or client) as requested by user.
def main():
    #creates parse objects and other options with defaults on some
